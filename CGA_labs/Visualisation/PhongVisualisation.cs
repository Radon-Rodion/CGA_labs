...
{
    public class PhongVisualisation : AbstractVisualisation
    {
        ...
        {
            var cos = Vector3.Dot(normalInPoint, _lightVector);
            var k = cos > 0 ? cos : 0;
        ...
        private byte[] GetColorFromNormalLightAndCamera(Vector3 normalInPoint, Vector3 cameraVector)
        {
            ...
            byte blue = (byte) Math.Min(ambient.b + diffuse.b + specular.b, 255);
            byte green = (byte) Math.Min(ambient.g + diffuse.g + specular.g, 255);
            byte red = (byte) Math.Min(ambient.r + diffuse.r + specular.r, 255);
            byte alpha = 255;
        ...
        protected override void DrawFace(WriteableBitmap bitmap, Model model, List<Vector3> face)
        {
            ...
            while (line01.y <= Math.Floor(pNCsArr[1].Point.Y))
            {
                ...
                var dNormal = (line01.normal - line02.normal) != Vector3.Zero
                    ? (line01.normal - line02.normal) / (line01.x - line02.x)
                    : Vector3.Zero;
                var dCamera = (line01.camera - line02.camera) != Vector3.Zero
                    ? (line01.camera - line02.camera) / (line01.x - line02.x)
                    : Vector3.Zero;
                for (int x = (int) line01.x; dx * x <= dx * line02.x; x += dx)
                {
                        ...
                        z < _zBuffer[x, (int) line01.y] && IsPointVisible(normal, camera))
                    {
                        ...
                        GetPixelColor = () => GetColorFromNormalLightAndCamera(normal, camera);
            ...
            while (line12.y <= Math.Floor(pNCsArr[2].Point.Y))
            {
                ...
                var dNormal = (line12.normal - line02.normal) != Vector3.Zero
                    ? (line12.normal - line02.normal) / (line12.x - line02.x)
                    : Vector3.Zero;
                var dCamera = (line12.camera - line02.camera) != Vector3.Zero
                    ? (line12.camera - line02.camera) / (line12.x - line02.x)
                    : Vector3.Zero;
